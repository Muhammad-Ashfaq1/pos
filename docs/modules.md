...
- **Aggregation**: [`TenantDashboardService`](../app/Services/TenantDashboardService.php) builds the payload — stat cards (total sales, collected, outstanding, orders, customers, products, low-stock, avg order, items sold), a revenue/orders trend, orders-by-status, payment-method split, top products, sales-by-category, customers-by-type, a revenue breakdown (net/tax/fees/discounts), recent orders, low-stock alerts, and an application overview (shop status, onboarding, currency, timezone, catalog/record counts).
- **Date filter**: a dropdown (today / yesterday / last 7 days / this month / this year / custom range) sets `?period=…` (plus `start`/`end` for custom); [`DashboardDateRange`](../app/Support/DashboardDateRange.php) resolves it and every time-based metric is scoped to that window (the headline change compares against the equal-length previous window; the trend chart buckets daily for spans ≤ 62 days, otherwise monthly). Catalog/inventory counts stay current-state. Seed demo data across the last 30 days with [`DashboardDemoSeeder`](../database/seeders/DashboardDemoSeeder.php).
