(function ($) {
  ...
  const alignCategoryActionsWithSearch = function (table) {
    if (window.PosListingToolbar && typeof window.PosListingToolbar.align === 'function') {
      window.PosListingToolbar.align(table, '#categoryTableActions');
    }
  };
  ...
  const initDataTable = function () {
    ...
    categoryTable = new DataTable($table[0], {
      ...
      drawCallback: function () {
        alignCategoryActionsWithSearch(this.api());
    ...
    alignCategoryActionsWithSearch(categoryTable);
  };
  ...
  const bindSaveForm = function (validator) {
    $form.on('submit', function (event) {
      ...
      if (window.appLoading && typeof window.appLoading.show === 'function') {
        window.appLoading.show('Saving category...');
      }
      ...
      window.PosConfirm.open({
        ...
        onConfirm: function () {
          return $.ajax({
            ...
            function (xhr) {
              throw new Error((xhr.responseJSON && xhr.responseJSON.message) || 'Unable to delete category.');
            }
          );
        }
      });
